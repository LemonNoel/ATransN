        ...
        for gan_step in range(args.gan_steps):
            ...
            if args.gan_n_critic > 0:
                if gan_step % args.gan_n_critic == 0:
                    ...
                    # random_z = (torch.rand_like(student_embed) * 2 - 1)
                    # fake_emb = generator(student_embed.detach(), random_z)
                    loss_g_self = -torch.mean(discriminator(student_embed.detach(), fake_emb))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gan_steps", type=int, default=0,
                        help="WGAN internal steps")
    ...
    parser.add_argument("--gan_clip_value", type=float, default=0.01,
                        help="clip value for discriminator weights in WGAN")
    parser.add_argument("--gan_n_critic", type=int, default=1,
                        help="number of iterations to update discriminator in WGAN")
    ...
    args.teacher_data_path.rstrip("/")
    args.teacher_data_path.rstrip("\\")
    args.student_data_path.rstrip("/")
    args.student_data_path.rstrip("\\")
    ...
    args.save_path = os.path.join(args.save_path, "WGAN_%s_T%s_S%s_GLR%.4f_GS%d_KLR%.4f_A%.4f_B%.4f_ED%d_MG%d_NS%d_SD%d_%s" % (
        args.kge_model,
